/// Google gVNIC (Virtual NIC) driver — DQO RDA queue format (x86_64 port).
///
...
/// RX completion descriptor (32 bytes)
const RxCompDesc = extern struct {
    rxdid_flags: u16, // bits[3:0]=rxdid, bit8=loopback, bit10=rx_error
    ptype_flags: u16, // bits[9:0]=ptype
    pktlen_gen: u16, // bits[13:0]=packet_len, bit14=generation
    hdrlen_flags: u16,
    desc_done_flags: u8, // bit0=done, bit1=eop
    status1: u8,
    reserved0: u8,
    ts_sub_nsecs: u8,
    buf_id: u16, // matches posted buf_id
    rsc_seg_len: u16,
    hash: u32,
    reserved1: u32,
    reserved2: u32,
...
var rx_comp_head: u16 = 0;
var rx_comp_gen: u1 = 0;
...
var rx_ring_head: u32 = 0;
var rx_ring_tail: u32 = 0;
...
// x86_64 has cache-coherent DMA — these are compiler barriers only.

/// Clean data cache for a range (CPU writes -> device reads via DMA)
...
/// Invalidate data cache for a range (device writes via DMA -> CPU reads)
...
/// Clean+invalidate cache to ensure CPU writes are visible to device.
...
fn allocDmaPage() struct { phys: u64, virt: [*]u8 } {
    ...
    const virt: [*]u8 = @ptrFromInt(hhdm.physToVirt(phys));
    // Zero it
...
fn allocDmaPages(n: usize) struct { phys: u64, virt: [*]u8 } {
    ...
    const virt: [*]u8 = @ptrFromInt(hhdm.physToVirt(first));
...
fn writeHex(val: u32) void {
    serial.print("{x}", .{val});
}

// ---- PCI MSI-X setup ----

// LAPIC MSI address base for x86_64
const LAPIC_MSI_ADDR: u64 = 0xFEE00000;
// MSI-X vectors start at 48 to avoid conflict with PIC IRQs (32-47)
const MSIX_VECTOR_BASE: u16 = 48;
...
fn setupMsix(dev: *const pci.PciDevice) u16 {
    ...
    const status = pci.pciRead16(pci_bus, pci_dev, pci_func, 0x06);
    if (status & (1 << 4) == 0) {
        serial.writeString("[gvnic] No PCI capabilities list\n");
        return 0;
    }

    var cap_off: u8 = pci.pciRead8(pci_bus, pci_dev, pci_func, 0x34) & 0xFC;
    var msix_cap_offset: u8 = 0;
    var iterations: u32 = 0;
    while (cap_off != 0 and iterations < 48) : (iterations += 1) {
        const cap_id = pci.pciRead8(pci_bus, pci_dev, pci_func, cap_off);
        serial.print("[gvnic] PCI cap at {x}: id={x}\n", .{ cap_off, cap_id });
        if (cap_id == 0x11) { // MSI-X
            msix_cap_offset = cap_off;
            break;
        }
        cap_off = pci.pciRead8(pci_bus, pci_dev, pci_func, cap_off + 1) & 0xFC;
    }

    if (msix_cap_offset == 0) {
        serial.writeString("[gvnic] MSI-X capability not found\n");
        return 0;
    }
    ...
    //   +8: pba_offset_bir (32)
    const msg_ctrl = pci.pciRead16(pci_bus, pci_dev, pci_func, msix_cap_offset + 2);
    const table_size = (msg_ctrl & 0x7FF) + 1; // bits[10:0] = N-1
    serial.print("[gvnic] MSI-X: {} vectors, msg_ctrl={x}\n", .{ table_size, msg_ctrl });

    const table_info = pci.pciRead32(pci_bus, pci_dev, pci_func, msix_cap_offset + 4);
    const table_bir = table_info & 0x7; // BAR index
    const table_offset = table_info & ~@as(u32, 0x7); // Offset within BAR
    serial.print("[gvnic] MSI-X table: BAR{} offset={x}\n", .{ table_bir, table_offset });

    // Get BAR address for the MSI-X table
    // On x86_64, BAR addresses from PCI config need HHDM mapping for MMIO access
    var table_bar_base: usize = switch (table_bir) {
        0 => @truncate(dev.bar0),
        2 => bar2, // Already resolved during init
        else => 0,
    };
    ...
    if (table_bir == 1) {
        const bar1_val = pci.pciRead32(pci_bus, pci_dev, pci_func, 0x14);
        const bar1_is_io = (bar1_val & 1) != 0;
        const bar1_type = (bar1_val >> 1) & 3;
        var bar1_addr: u64 = if (bar1_is_io) bar1_val & 0xFFFFFFFC else bar1_val & 0xFFFFFFF0;
        if (!bar1_is_io and bar1_type == 2) {
            // 64-bit BAR: read high 32 bits from next register (BAR2 at 0x18)
            const bar1_hi = pci.pciRead32(pci_bus, pci_dev, pci_func, 0x18);
            bar1_addr |= @as(u64, bar1_hi) << 32;
        }
        serial.print("[gvnic] BAR1 raw={x} addr={x}\n", .{ bar1_val, bar1_addr });
        if (bar1_addr != 0) {
            table_bar_base = @truncate(hhdm.physToVirt(bar1_addr));
        } else {
            serial.writeString("[gvnic] BAR1 not assigned, cannot access MSI-X table\n");
            return 0;
        }
    }
    if (table_bar_base == 0) {
        serial.print("[gvnic] MSI-X table BAR{} not mapped\n", .{table_bir});
        return 0;
    }
    ...
    // Set up MSI-X table entries with x86_64 LAPIC MSI address format.
    ...
    while (vec < table_size and vec < 8) : (vec += 1) {
        ...
        entry_lo.* = @truncate(LAPIC_MSI_ADDR); // LAPIC MSI base, target CPU 0
        entry_hi.* = 0; // Address high = 0 for LAPIC MSI
        entry_data.* = MSIX_VECTOR_BASE + vec; // Vector number (48+)
    ...
    pci.pciWrite16(pci_bus, pci_dev, pci_func, msix_cap_offset + 2, new_ctrl);

    const verify = pci.pciRead16(pci_bus, pci_dev, pci_func, msix_cap_offset + 2);
    serial.print("[gvnic] MSI-X enabled: msg_ctrl={x}\n", .{verify});
...
fn aqSubmit(cmd: *AdminCmd) bool {
    ...
    serial.writeString("[gvnic] AQ cmd: ");
    var wi: usize = 0;
    while (wi < 40) : (wi += 4) {
    ...
    serial.print("[gvnic] AQ slot={} prod={} event_before={}\n", .{ slot, aq_prod, event_before });
    ...
    while (spin < 10_000_000) : (spin += 1) {
        ...
        if (event >= aq_prod) {
            serial.print("[gvnic] AQ complete: event={} spins={}\n", .{ event, spin });
            ...
            const aq_status = @byteSwap(status_ptr.*);
            if (aq_status == 1) {
                return true;
            }
            serial.print("[gvnic] AQ cmd {x} failed: status={x}\n", .{ @byteSwap(cmd.opcode), aq_status });
            return false;
        }
        asm volatile ("pause");
    }
    serial.print("[gvnic] AQ cmd {x} timeout (event={} wanted={})\n", .{
        @byteSwap(cmd.opcode),
...
fn aqVerifyDriver() bool {
    ...
    serial.writeString("[gvnic] driver_info: ");
    var di: usize = 0;
    while (di < 24) : (di += 1) {
        writeHex(@as(u32, d[di]));
        serial.writeString(" ");
    }
    serial.writeString("\n");
...
fn aqDescribeDevice() bool {
    ...
    if (!aqSubmit(&cmd)) {
        ...
        serial.writeString("[gvnic] Desc buf check: ");
        var ci: usize = 0;
        while (ci < 32) : (ci += 1) {
            writeHex(@as(u32, d0[ci]));
            serial.writeString(" ");
        }
        serial.writeString("\n");
        ...
        if (check_tx > 0) {
            serial.writeString("[gvnic] Descriptor has data despite status=0, continuing\n");
        } else {
            return false;
        }
    } else {
    ...
    serial.print("[gvnic] MAC: {x:0>2}:{x:0>2}:{x:0>2}:{x:0>2}:{x:0>2}:{x:0>2}\n", .{
        mac[0], mac[1], mac[2], mac[3], mac[4], mac[5],
    });
    serial.print("[gvnic] MTU={} queues={} tx_entries={} rx_entries={} counters={}\n", .{
    ...
    while (opts_parsed < num_options and opt_offset + 8 <= PAGE_SIZE) : (opts_parsed += 1) {
        ...
        serial.print("[gvnic] Device option: id={x} len={}\n", .{ opt_id, opt_len });
        if (opt_id == DEV_OPT_DQO_RDA) {
            queue_format = QUEUE_FORMAT_DQO_RDA;
            serial.writeString("[gvnic] DQO RDA queue format supported\n");
        }
        // Advance past option header (8 bytes) + option data (opt_len bytes)
        opt_offset += 8 + @as(usize, opt_len);
    }
    if (queue_format == 0) {
        serial.writeString("[gvnic] WARNING: DQO RDA not in device options, trying anyway\n");
        queue_format = QUEUE_FORMAT_DQO_RDA;
    }
...
fn aqConfigureResources() bool {
    ...
    serial.print("[gvnic] Configuring: counters={} ntfy_blks={} msix_vecs={} format={x}\n", .{ actual_counters, num_ntfy_blks, msix_num_vectors, queue_format });
    ...
    if (num_ntfy_blks > 0) {
        ...
        serial.print("[gvnic] irq_db_addr={x} stride={} num={} msix_base=0\n", .{ irq_db_phys, irq_db_stride, num_ntfy_blks });
    }
...
fn aqCreateTxQueue() bool {
    ...
    serial.print("[gvnic] Creating TX queue: ring_size={}\n", .{ring_size});
    ...
    serial.print("[gvnic] TX queue created, doorbell={}\n", .{tx_db_index});
    return true;
}
...
fn aqCreateRxQueue() bool {
    ...
    serial.print("[gvnic] Creating RX queue: ring_size={}\n", .{ring_size});
    ...
    // Allocate RX completion ring (32 bytes per descriptor)
    ...
    serial.print("[gvnic] RX queue created, doorbell={}\n", .{rx_db_index});
    return true;
}
...
fn postRxBuffers() void {
    ...
    serial.print("[gvnic] RX buf[0]: id={} addr={x} hdr={x}\n", .{
    ...
    serial.print("[gvnic] {} RX buffers posted, tail={} db={x}\n", .{ NUM_RX_BUFS, rx_buf_tail, rx_db_index });
}
...
fn enableNotifyBlockIrqs() void {
    ...
    while (i < num_ntfy_blks) : (i += 1) {
        ...
        serial.print("[gvnic] Ntfy blk {}: irq_db_idx={} itr_val={x}\n", .{ i, db_idx_be, itr_val });
...
fn rearmNotifyBlock(ntfy_id: u32) void {
    ...
    const db_idx_be = @as(u32, entry_ptr[byte_off]) << 24 |
        ...
        @as(u32, entry_ptr[byte_off + 2]) << 8 |
        @as(u32, entry_ptr[byte_off + 3]);
    writeDb32(db_idx_be, GVE_ITR_NO_UPDATE_DQO | GVE_ITR_ENABLE_BIT_DQO | GVE_ITR_CLEAR_PBA_BIT_DQO);
}
...
/// Read a 64-bit BAR from PCI config space (handles 32-bit and 64-bit BARs).
fn readBar(bus: u8, device: u8, function: u8, bar_offset: u8) u64 {
    const bar_lo = pci.pciRead32(bus, device, function, bar_offset);
    const is_io = (bar_lo & 1) != 0;
    const bar_type = (bar_lo >> 1) & 3;

    var addr: u64 = if (is_io) bar_lo & 0xFFFFFFFC else bar_lo & 0xFFFFFFF0;
    if (!is_io and bar_type == 2) {
        // 64-bit BAR: high 32 bits in next register
        const bar_hi = pci.pciRead32(bus, device, function, bar_offset + 4);
        addr |= @as(u64, bar_hi) << 32;
    }
    return addr;
}
...
pub fn init(dev: *const pci.PciDevice) bool {
    // BAR0 from PCI scanner (already in dev struct)
    const bar0_phys = dev.bar0;

    // Read BAR2 from PCI config space (x86_64 scanner only reads BAR0)
    // BAR2 is at config register 0x18. If BAR0 is 64-bit, BAR2 is actually at 0x18.
    // If BAR0 is 32-bit, BAR1 might be 64-bit pushing BAR2 further.
    // Standard: BAR0=0x10, BAR1=0x14, BAR2=0x18, BAR3=0x1C, BAR4=0x20, BAR5=0x24
    // Check if BAR0 is 64-bit (consumes BAR0+BAR1 slots)
    const bar0_raw = pci.pciRead32(dev.bus, dev.device, dev.function, 0x10);
    const bar0_is_64bit = ((bar0_raw & 1) == 0) and (((bar0_raw >> 1) & 3) == 2);

    var bar2_phys: u64 = 0;
    if (bar0_is_64bit) {
        // BAR0 is 64-bit (occupies 0x10+0x14), so BAR2 starts at 0x18
        bar2_phys = readBar(dev.bus, dev.device, dev.function, 0x18);
    } else {
        // BAR0 is 32-bit at 0x10. Check if BAR1 (0x14) is 64-bit.
        const bar1_raw = pci.pciRead32(dev.bus, dev.device, dev.function, 0x14);
        const bar1_is_64bit = ((bar1_raw & 1) == 0) and (((bar1_raw >> 1) & 3) == 2);
        if (bar1_is_64bit) {
            // BAR1 occupies 0x14+0x18, so BAR2 starts at 0x1C
            bar2_phys = readBar(dev.bus, dev.device, dev.function, 0x1C);
        } else {
            // BAR0 32-bit, BAR1 32-bit, BAR2 at 0x18
            bar2_phys = readBar(dev.bus, dev.device, dev.function, 0x18);
        }
    }

    if (bar0_phys == 0 or bar2_phys == 0) {
        serial.writeString("[gvnic] Missing BAR0 or BAR2\n");
        return false;
    }

    // Map BARs via HHDM (physical MMIO addresses -> kernel virtual)
    bar0 = @truncate(hhdm.physToVirt(bar0_phys));
    bar2 = @truncate(hhdm.physToVirt(bar2_phys));

    serial.print("[gvnic] BAR0 phys={x} virt={x}\n", .{ bar0_phys, bar0 });
    serial.print("[gvnic] BAR2 phys={x} virt={x}\n", .{ bar2_phys, bar2 });

    // Check device status
    const status = readBe32(bar0 + REG_DEVICE_STATUS);
    serial.print("[gvnic] Device status: {x}\n", .{status});

    // Read max queues
    const max_tx = readBe32(bar0 + REG_MAX_TX_QUEUES);
    const max_rx = readBe32(bar0 + REG_MAX_RX_QUEUES);
    serial.print("[gvnic] Max queues: TX={} RX={}\n", .{ max_tx, max_rx });
    ...
    const pci_rev = pci.pciRead8(dev.bus, dev.device, dev.function, 0x08);
    serial.print("[gvnic] PCI revision: {x}\n", .{pci_rev});
    ...
    serial.print("[gvnic] AQ phys={x} PFN={x}\n", .{ aq_phys, aq_phys / PAGE_SIZE });
    ...
    serial.print("[gvnic] AQ readback: PFN={x} len_reg={x}\n", .{ aq_pfn_rb, aq_len_rb });

    // Verify driver compatibility (required before DESCRIBE_DEVICE on GCE firmware)
    if (!aqVerifyDriver()) {
        serial.writeString("[gvnic] VERIFY_DRIVER failed\n");
        return false;
    }

    // Describe device (get MAC, MTU, queue sizes)
    if (!aqDescribeDevice()) {
        serial.writeString("[gvnic] DESCRIBE_DEVICE failed\n");
        return false;
    }
    ...
    if (nvecs == 0) {
        serial.writeString("[gvnic] WARNING: MSI-X setup failed, queue creation may fail\n");
    }

    // Set IRQ to first MSI-X vector number
    if (nvecs > 0) {
        irq = MSIX_VECTOR_BASE; // First MSI-X vector (48)
        serial.print("[gvnic] IRQ set to MSI-X vector {}\n", .{irq});
    }

    // Configure resources (counters, IRQ doorbells, queue format)
    if (!aqConfigureResources()) {
        serial.writeString("[gvnic] CONFIGURE_RESOURCES failed\n");
        return false;
    }

    // Create TX queue
    if (!aqCreateTxQueue()) {
        serial.writeString("[gvnic] CREATE_TX_QUEUE failed\n");
        return false;
    }

    // Create RX queue
    if (!aqCreateRxQueue()) {
        serial.writeString("[gvnic] CREATE_RX_QUEUE failed\n");
        return false;
    }
    ...
    if (link_status & STATUS_LINK_UP != 0) {
        serial.writeString("[gvnic] Link is UP\n");
    } else {
        serial.writeString("[gvnic] Link is DOWN (will poll)\n");
    }

    // IRQ fallback: if MSI-X didn't set up, use legacy PCI IRQ line
    if (irq == 0) {
        // x86_64: PIC IRQs start at vector 32, use device's irq_line from PCI config
        irq = 32 + @as(u32, dev.irq_line);
    }
    serial.print("[gvnic] Using IRQ {} (MSI-X={})\n", .{ irq, @as(u8, if (irq >= MSIX_VECTOR_BASE) 1 else 0) });

    initialized = true;
    serial.print("[gvnic] Initialized (MAC {x:0>2}:{x:0>2}:{x:0>2}:{x:0>2}:{x:0>2}:{x:0>2})\n", .{
        mac[0], mac[1], mac[2], mac[3], mac[4], mac[5],
    });
...
pub fn transmit(data: []const u8) bool {
    ...
    if (tx_log_count < 3) {
        serial.writeString("[gvnic] TX TIMEOUT\n");
        tx_log_count += 1;
    }
...
var rx_log_count: u32 = 0;
pub fn handleIrq() void {
    _ = rx_log_count;
    // Process RX completions
    var processed: u32 = 0;
    while (processed < 32) : (processed += 1) {
        // Invalidate cache for this completion entry before reading
    ...
    // Re-arm RX notification block interrupt (ntfy_id=1)
    rearmNotifyBlock(1);
    // Re-arm TX notification block too (ntfy_id=0)
    rearmNotifyBlock(0);
}
