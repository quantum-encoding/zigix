...
pub fn bootPing(dst_ip: u32) void {
    ...
    if (!sendEchoRequest(dst_ip, 0x5A49, 1, payload)) {
        serial.writeString("[net]  ICMP: failed to send ping\n");
        return;
    }

    // Poll for reply using tick-based timeout (~2 seconds)
    const start = idt.getTickCount();
    while (idt.getTickCount() - start < 200 and !pending_reply) {
    ...
    if (pending_reply) {
        serial.writeString("[net]  ICMP: ping ");
        ethernet.writeIpAddr(dst_ip);
        serial.writeString(" seq=1 -> reply TTL=");
        writeDecimal(reply_ttl);
        serial.writeString("\n");
    } else {
        serial.writeString("[net]  ICMP: ping ");
        ethernet.writeIpAddr(dst_ip);
