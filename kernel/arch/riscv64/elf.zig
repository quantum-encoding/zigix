/// ELF64 loader for RISC-V -- parses ELF headers and maps PT_LOAD segments
...
/// is copied through identity mapping (RISC-V: phys == virt for kernel).
...
/// Sv39 user space ends at 2^38 = 0x4000000000
const USER_SPACE_END: u64 = 0x0000_0040_0000_0000;
...
pub fn getHeader(data: []const u8) ?*align(1) const Elf64Header {
    ...
    // Machine (RISC-V)
    if (header.e_machine != EM_RISCV) return null;
...
fn loadSegment(page_table: u64, data: []const u8, phdr: *align(1) const Elf64Phdr) !void {
    ...
    while (remaining > 0) {
        ...
        // Identity mapping: phys == virt, so we can write directly.
