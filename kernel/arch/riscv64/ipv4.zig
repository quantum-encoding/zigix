...
// Static network config — detect GCE vs QEMU at boot.
// GCE: IP from instance metadata (set in boot.zig), /20 subnet.
// QEMU SLIRP: 10.0.2.15, gateway 10.0.2.2, /24 subnet.
pub var our_ip: u32 = 0x0A00020F; // 10.0.2.15 (overridden for GCE)
pub var gateway_ip: u32 = 0x0A000202; // 10.0.2.2 (overridden for GCE)
pub var subnet_mask: u32 = 0xFFFFFF00; // 255.255.255.0 (overridden for GCE)
pub var dns_ip: u32 = 0x0A000203; // 10.0.2.3 (overridden for GCE)
...
fn sendFrame(dst_mac: [6]u8, ip_pkt: []const u8) bool {
    ...
    const len = ethernet.build(&frame, dst_mac, nic.mac, ethernet.ETH_P_IP, ip_pkt);
    if (len == 0) return false;
    return nic.transmit(frame[0..len]);
}
