...
pub const VIRTIO_MMIO_BASE: usize = 0x0a000000;
pub const VIRTIO_MMIO_STRIDE: usize = 0x200;
pub const VIRTIO_MMIO_COUNT: usize = 32;
pub const VIRTIO_IRQ_BASE: u32 = 48; // SPI 16 = GIC IRQ 48
...
const CACHE_LINE_SIZE: u64 = 64; // All Cortex-A cores use 64-byte cache lines

/// Clean and invalidate a range of physical memory from the D-cache.
/// Use BEFORE DMA for CPU→device transfers (ensures device sees CPU writes).
/// Use AFTER DMA for device→CPU transfers (ensures CPU sees device writes).
pub fn dmaCleanRange(phys: u64, size: u64) void {
    var addr = phys & ~(CACHE_LINE_SIZE - 1);
    const end = phys + size;
    ...
    // DSB ensures all cache maintenance completes; ISB flushes the pipeline
    // so subsequent MMIO writes see the cleaned data.
    asm volatile ("dsb sy" ::: .{ .memory = true });
    asm volatile ("isb" ::: .{ .memory = true });
}

/// Invalidate a range of physical memory from the D-cache.
/// Use BEFORE and AFTER DMA for device→CPU transfers.
/// Uses civac (clean+invalidate) for safety — plain ivac can lose dirty data.
pub fn dmaInvalidateRange(phys: u64, size: u64) void {
    var addr = phys & ~(CACHE_LINE_SIZE - 1);
    const end = phys + size;
    while (addr < end) : (addr += CACHE_LINE_SIZE) {
        asm volatile ("dc civac, %[addr]"
...
pub fn finishInit(base: usize) void {
    mmioWrite32(base, REG_STATUS, STATUS_ACKNOWLEDGE | STATUS_DRIVER | STATUS_DRIVER_OK);
}
