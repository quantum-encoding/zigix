...
/// is copied through identity mapping (ARM64: phys == virt).
/// BSS (p_memsz > p_filesz) is zero-filled.
...
pub fn loadElf(page_table: u64, data: []const u8) !ElfInfo {
    ...
    uart.print("[elf] Loaded {} segments, entry={x}\n", .{ info.segments_loaded, info.entry });
...
pub fn getHeader(data: []const u8) ?*align(1) const Elf64Header {
    ...
    // Machine (AArch64)
    if (header.e_machine != EM_AARCH64) return null;
...
fn loadSegment(page_table: u64, data: []const u8, phdr: *align(1) const Elf64Phdr) !void {
    ...
    var addr = seg_start;
    while (addr < seg_end) : (addr += PAGE_SIZE) {
        const page = pmm.allocPage() orelse return error.OutOfMemory;
        zeroPage(page);
        vmm.mapPage(vmm.PhysAddr.from(page_table), vmm.VirtAddr.from(addr), vmm.PhysAddr.from(page), .{
            .user = true,
            .writable = writable,
    ...
    while (remaining > 0) {
        ...
        // translate() returns the physical address.
        // ARM64 identity mapping: phys == virt, so we can write directly.
        if (vmm.translate(vmm.PhysAddr.from(page_table), vmm.VirtAddr.from(vaddr))) |phys| {
            const dst: [*]u8 = @ptrFromInt(phys.toInt());
            for (0..chunk) |j| {
