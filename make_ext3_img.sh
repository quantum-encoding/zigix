#!/bin/bash
# Create an ext3-formatted disk image for Zigix.
# Builds userspace programs and all zig_core_utils, bundles them into a 1GB ext3 image.
# Same structure as make_ext2_img.sh but produces ext3 with JBD2 journal.
# Skips rebuilds when binaries are already up-to-date (checks source timestamps).
set -e

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
COREUTILS_DIR="$(cd "$SCRIPT_DIR/../programs/zig_core_utils" && pwd)"
IMG="$SCRIPT_DIR/test_ext3.img"
ANY_REBUILT=0
...
# Build freestanding userspace programs (only if sources changed)
...
# Cross-compile ALL core utils from zig_core_utils for x86_64-linux-musl
...
if [ "$ANY_REBUILT" -eq 0 ] && [ -f "$IMG" ]; then
    echo ""
    echo "ext3 image up to date, skipping regeneration"
    echo "Done: $(ls -lh "$IMG" | awk '{print $5}') ext3 image (cached)"
    exit 0
fi
...
# Copy zinit and zlogin (freestanding only, no musl equivalent)
...
# Copy all cross-compiled core utils (musl)
for dir in "$COREUTILS_DIR"/z*/; do
    [ -d "$dir" ] || continue
    util=$(basename "$dir")
...
# Copy freestanding versions LAST — they use direct int $0x80 syscalls
# and work correctly in Zigix (musl versions need io_uring/epoll for std.Io)
...
if [ -f "$ZIG_LINUX_DIR/zig" ] && [ -d "$ZIG_LINUX_DIR/lib" ]; then
    ...
    # Copy only the libc subdirs needed for x86_64-linux-musl target
    ...
    # No lib/ — just add binary to /bin/ as before
    echo "Adding Zig compiler (binary only) from $ZIG_LINUX_DIR..."
    cp "$ZIG_LINUX_DIR/zig" "$EXTRA_DIR/" 2>/dev/null || true
fi

# Build ext3 image with shell + extras + test scripts + optional zig tree
SCRIPTS_DIR="$SCRIPT_DIR/test_scripts"
echo ""
echo "Generating ext3 image..."
if [ -n "$ZIG_TREE" ]; then
    python3 "$SCRIPT_DIR/make_ext3_img.py" "$IMG" "$SHELL_BIN" "$EXTRA_DIR" "$SCRIPTS_DIR" "$ZIG_TREE"
else
    python3 "$SCRIPT_DIR/make_ext3_img.py" "$IMG" "$SHELL_BIN" "$EXTRA_DIR" "$SCRIPTS_DIR"
fi

rm -rf "$EXTRA_DIR"
[ -n "$ZIG_TREE" ] && rm -rf "$ZIG_TREE"

echo "Done: $(ls -lh "$IMG" | awk '{print $5}') ext3 image"
