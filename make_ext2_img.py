#!/usr/bin/env python3
"""Create an ext3 filesystem image with test files, shell binary, and extra binaries.

Usage: make_ext2_img.py <output.img> [shell_binary] [extra_bins_dir] [scripts_dir] [zig_tree_dir]

Writes raw ext2/ext3 structures — no external tools required.
Block size: 4096 bytes, 1 block group, 1GB image, 16384 inodes.
Includes 4MB JBD2 journal (inode 8) for ext3 journaling.
...
Layout (4096-byte blocks):
  ...
  Blocks 4-515: inode table (16384 inodes × 128 bytes = 512 blocks)
  Block 516+: data blocks
  Overflow zone (32768+): binaries, Zig tree, ext3 journal
"""
...
INODE_SIZE = 128
INODE_TABLE_BLOCKS = (INODES_PER_GROUP * INODE_SIZE) // BLOCK_SIZE  # 1024
FIRST_DATA_BLOCK = 4 + INODE_TABLE_BLOCKS  # Block 1028
BITMAP_CAPACITY = BLOCK_SIZE * 8  # 32768 — max blocks one bitmap block can track
...
def main():
    if len(sys.argv) < 2:
        print("Usage: make_ext2_img.py <output.img> [shell_binary] [extra_bins_dir] [scripts_dir] [zig_tree_dir]")
        sys.exit(1)
    ...
    # Bitmap zone: blocks FIRST_DATA_BLOCK .. BITMAP_CAPACITY-1 (kernel can alloc/free)
    # Overflow zone: blocks BITMAP_CAPACITY .. TOTAL_BLOCKS-1 (read-only at runtime)
    next_block = FIRST_DATA_BLOCK
    high_block = BITMAP_CAPACITY  # overflow zone starts here
    use_high_zone = False         # toggled for large file trees
    next_inode = 12  # First non-reserved inode (1-10 reserved, 11=lost+found)

    def alloc_block():
        nonlocal next_block, high_block
        if use_high_zone:
            b = high_block
            high_block += 1
            if b >= TOTAL_BLOCKS:
                print(f"ERROR: overflow zone exhausted (block {b}, max {TOTAL_BLOCKS})")
                sys.exit(1)
        else:
            b = next_block
            next_block += 1
            if b >= BITMAP_CAPACITY:
                print(f"ERROR: bitmap zone exhausted (block {b}, max {BITMAP_CAPACITY})")
                sys.exit(1)
        return b
        ...
        for idx, (inode, name, file_type) in enumerate(entries_list):
            ...
            if is_last or len(current_block_data) + entry_min_size > BLOCK_SIZE:
                if not is_last and len(current_block_data) + entry_min_size > BLOCK_SIZE:
                    # Current block is full — extend last entry's rec_len to fill block
                    if current_block_data:
                        remaining = BLOCK_SIZE - len(current_block_data)
                        if remaining > 0:
                            # Walk back to find the last entry's rec_len field and extend it
                            # rec_len is a u16 at offset 4 within each entry
                            pos = 0
                            last_pos = 0
                            while pos < len(current_block_data):
                                last_pos = pos
                                entry_rec_len = struct.unpack_from('<H', current_block_data, pos + 4)[0]
                                pos += entry_rec_len
                            # Extend the last entry's rec_len to absorb remaining space
                            old_rec_len = struct.unpack_from('<H', current_block_data, last_pos + 4)[0]
                            new_rec_len = old_rec_len + remaining
                            current_block_data = (current_block_data[:last_pos + 4] +
                                                  struct.pack('<H', new_rec_len) +
                                                  current_block_data[last_pos + 6:])
                            # Pad with zeros to fill the block
                            current_block_data += b'\x00' * remaining
                        blk = alloc_block()
    ...
    def add_tree(host_dir, self_ino, parent_ino):
        """Recursively add host directory contents to the ext2 image.
    ...
    hello_data = b"Hello from ext2!\n"
    test2_data = b"Nested file works\n"

    # /etc/motd for grep demos
    motd_data = b"Welcome to Zigix!\nA minimal Unix-like OS written in Zig.\nPowered by the Quantum Zig Forge software stack.\n"
    ...
    # /tmp directory with pre-created Zig cache subdirectories
    # The Zig compiler needs /tmp/zig-global/{b,h,p,tmp} and /tmp/zig-cache/{b,h,p,tmp,z}
    zig_global_ino = alloc_inode()
    zig_global_b_ino = alloc_inode()
    zig_global_h_ino = alloc_inode()
    zig_global_p_ino = alloc_inode()
    zig_global_tmp_ino = alloc_inode()
    zig_global_o_ino = alloc_inode()
    zig_cache_ino = alloc_inode()
    zig_cache_b_ino = alloc_inode()
    zig_cache_h_ino = alloc_inode()
    zig_cache_p_ino = alloc_inode()
    zig_cache_tmp_ino = alloc_inode()
    zig_cache_z_ino = alloc_inode()
    zig_cache_o_ino = alloc_inode()

    # Empty directory helper
    def make_empty_dir(self_ino, parent_ino):
        blk = alloc_block()
        entries = b''
        entries += make_dir_entry(self_ino, '.', FT_DIR)
        remaining = BLOCK_SIZE - len(entries)
        entries += make_dir_entry(parent_ino, '..', FT_DIR, rec_len=remaining)
        write_block(blk, entries)
        return blk

    # /tmp/zig-global subdirectories (empty)
    zg_b_blk = make_empty_dir(zig_global_b_ino, zig_global_ino)
    zg_h_blk = make_empty_dir(zig_global_h_ino, zig_global_ino)
    zg_p_blk = make_empty_dir(zig_global_p_ino, zig_global_ino)
    zg_tmp_blk = make_empty_dir(zig_global_tmp_ino, zig_global_ino)
    zg_o_blk = make_empty_dir(zig_global_o_ino, zig_global_ino)

    # /tmp/zig-cache subdirectories (empty)
    zc_b_blk = make_empty_dir(zig_cache_b_ino, zig_cache_ino)
    zc_h_blk = make_empty_dir(zig_cache_h_ino, zig_cache_ino)
    zc_p_blk = make_empty_dir(zig_cache_p_ino, zig_cache_ino)
    zc_tmp_blk = make_empty_dir(zig_cache_tmp_ino, zig_cache_ino)
    zc_z_blk = make_empty_dir(zig_cache_z_ino, zig_cache_ino)
    zc_o_blk = make_empty_dir(zig_cache_o_ino, zig_cache_ino)

    # /tmp/zig-global directory
    zg_block = alloc_block()
    zg_entries = b''
    zg_entries += make_dir_entry(zig_global_ino, '.', FT_DIR)
    zg_entries += make_dir_entry(tmp_ino, '..', FT_DIR)
    zg_entries += make_dir_entry(zig_global_b_ino, 'b', FT_DIR)
    zg_entries += make_dir_entry(zig_global_h_ino, 'h', FT_DIR)
    zg_entries += make_dir_entry(zig_global_p_ino, 'p', FT_DIR)
    zg_entries += make_dir_entry(zig_global_tmp_ino, 'tmp', FT_DIR)
    zg_remaining = BLOCK_SIZE - len(zg_entries)
    zg_entries += make_dir_entry(zig_global_o_ino, 'o', FT_DIR, rec_len=zg_remaining)
    write_block(zg_block, zg_entries)

    # /tmp/zig-cache directory
    zc_block = alloc_block()
    zc_entries = b''
    zc_entries += make_dir_entry(zig_cache_ino, '.', FT_DIR)
    zc_entries += make_dir_entry(tmp_ino, '..', FT_DIR)
    zc_entries += make_dir_entry(zig_cache_b_ino, 'b', FT_DIR)
    zc_entries += make_dir_entry(zig_cache_h_ino, 'h', FT_DIR)
    zc_entries += make_dir_entry(zig_cache_p_ino, 'p', FT_DIR)
    zc_entries += make_dir_entry(zig_cache_tmp_ino, 'tmp', FT_DIR)
    zc_entries += make_dir_entry(zig_cache_z_ino, 'z', FT_DIR)
    zc_remaining = BLOCK_SIZE - len(zc_entries)
    zc_entries += make_dir_entry(zig_cache_o_ino, 'o', FT_DIR, rec_len=zc_remaining)
    write_block(zc_block, zc_entries)

    # /tmp directory with zig-global and zig-cache
    tmp_block = alloc_block()
    tmp_entries = b''
    tmp_entries += make_dir_entry(tmp_ino, '.', FT_DIR)
    tmp_entries += make_dir_entry(2, '..', FT_DIR)
    tmp_entries += make_dir_entry(zig_global_ino, 'zig-global', FT_DIR)
    tmp_remaining = BLOCK_SIZE - len(tmp_entries)
    tmp_entries += make_dir_entry(zig_cache_ino, 'zig-cache', FT_DIR, rec_len=tmp_remaining)
    write_block(tmp_block, tmp_entries)

    # Write inodes for cache directories
    write_inode(zig_global_ino, 0o40755, BLOCK_SIZE, 1, [zg_block], nlink=7)  # ., b/.., h/.., p/.., tmp/.., o/..
    write_inode(zig_global_b_ino, 0o40755, BLOCK_SIZE, 1, [zg_b_blk], nlink=2)
    write_inode(zig_global_h_ino, 0o40755, BLOCK_SIZE, 1, [zg_h_blk], nlink=2)
    write_inode(zig_global_p_ino, 0o40755, BLOCK_SIZE, 1, [zg_p_blk], nlink=2)
    write_inode(zig_global_tmp_ino, 0o40755, BLOCK_SIZE, 1, [zg_tmp_blk], nlink=2)
    write_inode(zig_global_o_ino, 0o40755, BLOCK_SIZE, 1, [zg_o_blk], nlink=2)
    write_inode(zig_cache_ino, 0o40755, BLOCK_SIZE, 1, [zc_block], nlink=8)  # ., b/.., h/.., p/.., tmp/.., z/.., o/..
    write_inode(zig_cache_b_ino, 0o40755, BLOCK_SIZE, 1, [zc_b_blk], nlink=2)
    write_inode(zig_cache_h_ino, 0o40755, BLOCK_SIZE, 1, [zc_h_blk], nlink=2)
    write_inode(zig_cache_p_ino, 0o40755, BLOCK_SIZE, 1, [zc_p_blk], nlink=2)
    write_inode(zig_cache_tmp_ino, 0o40755, BLOCK_SIZE, 1, [zc_tmp_blk], nlink=2)
    write_inode(zig_cache_z_ino, 0o40755, BLOCK_SIZE, 1, [zc_z_blk], nlink=2)
    write_inode(zig_cache_o_ino, 0o40755, BLOCK_SIZE, 1, [zc_o_blk], nlink=2)
    ...
    if has_bin:
        ...
        # Put /bin/ contents in overflow zone — they're read-only at runtime
        # and would exhaust the bitmap zone (120+ musl binaries at ~3MB each)
        use_high_zone = True
    ...
    if len(sys.argv) >= 6 and os.path.isdir(sys.argv[5]):
        ...
        zig_ino = alloc_inode()
        use_high_zone = True  # large files go to overflow zone (blocks >= 32768)
        ...
        use_high_zone = False  # back to bitmap zone for remaining allocations
    ...
    # /swapfile — 16 MiB pre-allocated swap file (zeroed)
    swapfile_ino = alloc_inode()
    swapfile_size = 16 * 1024 * 1024  # 16 MiB
    swapfile_data = b'\x00' * swapfile_size
    swapfile_blocks, swapfile_indirect, swapfile_dindirect = alloc_file_blocks(swapfile_data)
    write_file_data(swapfile_data, swapfile_blocks, swapfile_indirect, swapfile_dindirect)
    print(f"  Swapfile: {swapfile_size // 1024 // 1024} MiB ({len(swapfile_blocks)} blocks)")
    ...
    write_inode(tmp_ino, 0o41777, BLOCK_SIZE, 1, [tmp_block], nlink=4)  # /tmp sticky bit + zig-global/.., zig-cache/..
    write_inode(hello_zig_ino, 0o100644, len(hello_zig_data), 1, [hello_zig_block])
    write_inode(swapfile_ino, 0o100600, swapfile_size, len(swapfile_blocks),
                swapfile_blocks[:12], indirect=swapfile_indirect, dindirect=swapfile_dindirect)
    ...
    # --- Allocate ext3 journal (in overflow zone) ---
    JOURNAL_BLOCKS = 1024  # 4 MB journal
    journal_start = high_block
    high_block += JOURNAL_BLOCKS
    if high_block > TOTAL_BLOCKS:
        print(f"ERROR: overflow zone exhausted for journal (need {high_block}, max {TOTAL_BLOCKS})")
        sys.exit(1)

    # Write JBD2 superblock at first journal block (big-endian)
    jsb = struct.pack('>IIIIII',
        0xC03B3998,      # magic
        4,               # blocktype (SUPERBLOCK_V2)
        0,               # sequence (clean)
        4096,            # blocksize
        JOURNAL_BLOCKS,  # maxlen
        1,               # first usable block
    )
    jsb_block = bytearray(BLOCK_SIZE)
    jsb_block[:len(jsb)] = jsb
    write_block(journal_start, jsb_block)

    # Write journal inode (inode 8) — slot already reserved in inode bitmap
    journal_size = JOURNAL_BLOCKS * BLOCK_SIZE
    write_inode(8, 0o100600, journal_size, JOURNAL_BLOCKS, [journal_start], nlink=1)
    print(f"  ext3 journal: {JOURNAL_BLOCKS} blocks ({journal_size // (1024*1024)} MB) at block {journal_start}")
    ...
    # Free blocks in bitmap zone (what the kernel can actually allocate)
    free_blocks = BITMAP_CAPACITY - next_block
    ...
    sb_data = struct.pack('<13I 6H 4I 2H',
        ...
        2,                  # s_log_frag_size
        BITMAP_CAPACITY,    # s_blocks_per_group (matches bitmap block capacity)
        BITMAP_CAPACITY,    # s_frags_per_group
    ...
    # ext3 journal fields (rev 1 dynamic superblock)
    struct.pack_into('<I', sb, 0x4C, 1)       # s_rev_level = 1 (dynamic)
    struct.pack_into('<I', sb, 0x54, 11)      # s_first_ino = 11
    struct.pack_into('<H', sb, 0x58, 128)     # s_inode_size = 128
    struct.pack_into('<I', sb, 0x5C, 0x0004)  # s_feature_compat = HAS_JOURNAL
    struct.pack_into('<I', sb, 0xE0, 8)       # s_journal_inum = 8
    ...
    overflow_used = high_block - BITMAP_CAPACITY
    total_used = next_block + overflow_used
    print(f"Created {sys.argv[1]}: {TOTAL_BLOCKS} blocks, {BLOCK_SIZE}-byte blocks ({TOTAL_BLOCKS * BLOCK_SIZE // (1024*1024)} MB)")
    print(f"  Inodes: {INODES_PER_GROUP} ({free_inodes} free)")
    print(f"  Bitmap zone: blocks 0-{BITMAP_CAPACITY-1} ({next_block} used, {free_blocks} free for runtime)")
    print(f"  Overflow zone: blocks {BITMAP_CAPACITY}-{high_block-1} ({overflow_used} used, read-only)")
    used_mb = total_used * BLOCK_SIZE / (1024 * 1024)
    print(f"  Used space: {used_mb:.1f} MB of {TOTAL_BLOCKS * BLOCK_SIZE // (1024*1024)} MB")
    files = "/hello.txt, /testdir/test2.txt, /etc/motd, /lost+found/"
    if shell_binary:
        files += f", /bin/zsh ({len(shell_binary)} bytes)"
    for name, ino, blocks, indirect, dindirect, data in extra_bin_info:
        files += f", /bin/{name} ({len(data)} bytes)"
