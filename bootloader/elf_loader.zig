/// ELF64 Loader for UEFI Bootloader
///
/// Two-phase loading:
///   Phase 1 (during Boot Services): Parse and validate ELF headers from a
///     memory buffer. No allocation — the kernel file is already loaded.
///   Phase 2 (after ExitBootServices): Copy PT_LOAD segments from the buffer
///     to their ELF-specified physical addresses. At this point all RAM is
///     available since UEFI firmware has released its allocations.
///
/// This avoids conflicts with UEFI's memory reservation at the kernel's
/// load address (0x40080000 on QEMU virt).
...
// ELF64 structures (match spec, little-endian)
...
/// Maximum PT_LOAD segments we support
const MAX_SEGMENTS = 8;

/// Information about a single PT_LOAD segment to be placed after ExitBootServices.
pub const SegmentInfo = struct {
    file_offset: u64, // Offset into the ELF file buffer
    vaddr: u64, // Target virtual (= physical) address
    filesz: u64, // Bytes to copy from file
    memsz: u64, // Total memory size (filesz + BSS)
};
...
/// Call this during UEFI Boot Services.
pub fn parseKernel(data: []const u8) LoadError!ParseResult {
    ...
    while (i < header.e_phnum) : (i += 1) {
        ...
        // Validate file data is within buffer
        ...
        if (seg_start < result.phys_base) result.phys_base = seg_start;
        if (seg_end > result.phys_end) result.phys_end = seg_end;
...
/// The `data` buffer must still be in memory (it was allocated via UEFI).
pub fn placeSegments(data: [*]const u8, parse: *const ParseResult) void {
    // Zero the entire kernel region first (covers BSS for all segments)
    ...
    while (i < parse.segment_count) : (i += 1) {
        ...
        const dest: [*]u8 = @ptrFromInt(seg.vaddr);
...
fn validateHeader(data: []const u8) LoadError!*const Elf64Header {
    ...
    if (header.e_machine != EM_AARCH64) return error.WrongMachine;
